...
carpool matchings and outputs the best of them.
...
connected component.
It is easy to see that the resulting graph is a forest of in-trees.
Each of these in-trees can be, in turn, decomposed into two disjoint
feasible carpool matchings.
This can be done, for example, by coloring each such in-tree with two
colors, say red and blue, and then consider the two solutions: one where the
...
\For{every connected component $C_i = (V_i, A_i) \in G'$}{	
	...
	Decompose the remaining in-tree into two solutions, $M^i_1$,
	$M^i_2$
	\\
...
\subref{subfloat:spanning} an in-tree:
